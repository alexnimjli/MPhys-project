...
for i in range(interval):
    ...
    for j in range(l_steps):
        ...
        E_vallist_sorted[j].append(Teig_vals_sorted[j])

energy_time = time.time()
print "Time to get all energies: ", energy_time - start_time

# for i in range(l_steps):
#     pylab.plot(B_axis, E_vallist_sorted[i])
# pylab.show()


# print "\n\n\n\n"
# print E_veclist[1][99]
# print (numpy.dot(T_matrix, E_veclist[1][99]))/E_vallist[1][99]
# match = 0
# for i in range(l_steps):
#     if str(((numpy.dot(T_matrix, E_veclist[1][99]))/E_vallist[1][99])[i]) == str(E_veclist[1][99][i]):
#         match += 1
# print "match : ", match
# print "dimension of the vector is", E_veclist[1][99].shape

# for i in range(l_steps):
#     pylab.plot(B_axis, E_vallist_sorted[i])

# pylab.title("%s atom system" %(atoms))
...
#         expoE_T = 0
#         for j in range(l_steps):
#             # expoE_T += (numpy.dot(numpy.transpose(random_veclist[i]), E_veclist[j][i])[0])**2 * numpy.exp(-(E_vallist[j][i])/ (kb * T_temp))
...
# list_random = []
# for i in range(l_steps):
#     ran = numpy.random.rand(l_steps, 1)
#     norm = numpy.linalg.norm(ran)
#     random = ran/norm
#     listr.append(random)
    ...
    for i in range(interval): #this gets all the energies for a given B and T
        ...
        for j in range(l_steps):
            for k in range(l_steps):
                # expoE_T += (numpy.dot(numpy.transpose(list_random[k]), E_veclist[j][i])[0])**2 * numpy.exp(-(E_vallist[j][i])/ (kb * T_temp))
                # expoE_T += (numpy.dot(numpy.transpose(listr[k]), E_veclist[j][i])[0])**2 * numpy.exp(-(E_vallist[j][i])/ (kb * T_temp))
                expoE_T += (numpy.absolute((numpy.dot(numpy.transpose(listr[k]), E_veclist[j][i])[0])))**2 * numpy.exp(-(E_vallist[j][i])/ (kb * T_temp))
                # expoE_T += (numpy.dot(numpy.transpose(random_veclist[k]), E_veclist[j][i])[0])**2 * numpy.exp(-(E_vallist[j][i])/(kb*T_temp))
        Zpf_T[w].append(expoE_T)
    T_axis.append(T_temp)
